...
AlashLEDMatrix8x8::AlashLEDMatrix8x8(uint8_t N, uint8_t ssPin, uint8_t flags, uint8_t* fb):
	...
	ssPin(ssPin)
{
	...
	setEnabled(false);
	setIntensity(0);
	_sendCommand(AlashLEDMatrix8x8::TEST);			//no test
	_sendCommand(AlashLEDMatrix8x8::DECODE);			//no decode
	_sendCommand(AlashLEDMatrix8x8::SCAN_LIMIT | 7);	//all lines
}

AlashLEDMatrix8x8::~AlashLEDMatrix8x8()
{
	if (selfAllocated)
		delete[] frameBuffer;
}

void AlashLEDMatrix8x8::setPixel(int16_t x, int16_t y, bool enabled)
{
...
bool AlashLEDMatrix8x8::getPixel(int16_t x, int16_t y) const
{
	uint8_t* p = _getBufferPtr(x, y);
	if (!p)
		return false;
...
void AlashLEDMatrix8x8::setColumn(int16_t x, uint8_t value)
{
...
void AlashLEDMatrix8x8::setEnabled(bool enabled)
{
...
void AlashLEDMatrix8x8::setIntensity(uint8_t level)
{
 ...
 */
void AlashLEDMatrix8x8::setScanLimit(uint8_t limit)
{
	//7 == all digits
 ...
 * The command is sent to all drivers.
 */
void AlashLEDMatrix8x8::setDecode(uint8_t mask)
{
 ...
 */
void AlashLEDMatrix8x8::setDigit(uint16_t digit, uint8_t value, bool dot)
{
	if (digit >= (8*N))
		return;
...
void AlashLEDMatrix8x8::_sendCommand(uint16_t command)
{
...
void AlashLEDMatrix8x8::_displayRow(uint8_t row)
{
...
uint8_t* AlashLEDMatrix8x8::_getBufferPtr(int16_t x, int16_t y) const
{
	if ((y < 0) or (y >= 8))
		return nullptr;
	if ((x < 0) or (x >= (8*N)))
...
void AlashLEDMatrix8x8::display()
{
	for (uint8_t y = 0; y < 8; y++)
	{
		_displayRow(y);
...
void AlashLEDMatrix8x8::scroll(scrollDirection direction, bool wrap)
{
	switch( direction )
	{
